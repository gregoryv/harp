...
func sendARP(ip net.IP) error {
	...
	if err != nil {
		return fmt.Errorf("Error finding interface %s: %v. Check interface name.", InterfaceName, err)
	}

	srcIP, err := getInterfaceIPv4(iface)
	if err != nil {
		return fmt.Errorf("Could not get source IP for %s: %v. Check IP configuration.", InterfaceName, err)
	}
	...
	fmt.Printf("Sending ARP Request from %s (%s) to resolve %s on interface %s...\n",
		iface.HardwareAddr, srcIP, ip, InterfaceName)
...
// The target IPv4 address to resolve (e.g., your router)
